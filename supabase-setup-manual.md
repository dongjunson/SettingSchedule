...
-- 4. Income Statements 테이블 생성 (손익계산서)
...
-- 6. 로그인 검증 RPC (클라이언트는 해시를 읽지 않고 함수로만 검증)
...
-- 8. 정책(Policy) 설정 - 인증된 사용자(로그인 후 Supabase Auth 세션)만 접근
...
왼쪽 사이드바의 **Table Editor**를 클릭하여 `sites`, `timeline_items`, `checklist_items`,
`income_statements`, `income_statement_items` 테이블이 정상적으로 생성되었는지 확인합니다.
